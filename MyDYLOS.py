...
# $Id: MyDYLOS.py,v 2.8 2017/02/18 14:50:41 teus Exp teus $
...
__options__ = [
    'input','type','usbid','file','firmware',
...
def registrate():
    ...
    Conf['input'] = False
    if (Conf['type'] == None) or (Conf['type'][0:8].lower() != 'dylos dc'):
        return False
    if not open_serial():
        return False
    Conf['input'] = True
...
def Add():
    ...
    return { "time": int(time()), "pm_25": bin_data[0], "pm_10": bin_data[1] }
