...
# $Id: MyWebDB.py,v 1.3 2021/08/26 15:48:06 teus Exp teus $
...
__modulename__='$RCSfile: MyWebDB.py,v $'[10:-4]
    ...
    if (Conf['fd'] == None) or (not Conf['fd']):
        ...
        for credit in ['hostname','user','password','db']:
            ...
            try:
                ...
                pass
        Conf['log'](WHERE(),'INFO','Using database %s on host %s, user %s credits.' % (Conf['db'],Conf['hostname'],Conf['user']))
        if (Conf['hostname'] != 'localhost') and ((not net['module']) or (not net['connected'])):
            Conf['log'](WHERE(True),'ERROR',"Access database %s / %s."  % (Conf['hostname'], Conf['db']))      
            Conf['output'] = False
            return False
        for M in ('user','password','hostname','db'):
            if (not M in Conf.keys()) or not Conf[M]:
                Conf['log'](WHERE(True),'ERROR',"Define WEB details and credentials.")
                Conf['output'] = False
                return False
...
def db_query(query,answer):
    ...
    try:
        ...
        Conf['log'](WHERE(True),'ERROR',"Failure type: %s; value: %s" % (sys.exc_info()[0],sys.exc_info()[1]) )
...
def db_table(table):
    ...
    if (not 'fd' in Conf.keys()) and not Conf['fd']:
        try: db_connect()
            ...
            Conf['log'](WHERE(True),'FATAL','No MySQL WEB connection available')
            return False
