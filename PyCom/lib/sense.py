...
# $Id: sense.py,v 1.1 2018/03/15 21:03:50 teus Exp teus $
#
...
def setup():
  ...
  display("MySense LoPy",0,0,True)
  myID = binascii.hexlify(unique_id()).decode('utf-8')
  display("s/n " + myID, 0, 16, False)
  dust = 'none'
  if sensor_sds011: dust = 'SDS011'
  meteo = 'none'
  if sensor_bme280: meteo = 'BME280'
  display("dust:  " + dust, 0, 30, False)
  display("meteo: " + meteo, 0, 44, False)
  sleep(30)
  ...
  while True:
    ...
    if sensor_bme280:
      try:
        ...
        if use_oled: i2c.init(0, pins=(B_SDA,B_SCL))
    ...
    if meteo:
      display("   C rh%% pHa   ", 0, 34, False)
      display("  o",              0, 30, False,False)
      display("%4.1f %2d %4d " % (t,h,p),0, 48, False)
    toSleep = time.time()
